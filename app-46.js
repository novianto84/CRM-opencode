...
window.addEventListener('error', (event) => {
  ...
  const where = event.lineno ? ` [${file} baris ${event.lineno}]` : '';
  showToast(`Galat halaman: ${event.message}${where}`, true);
});
...
const renderDbContactRows = (relations, allContacts) => {
  const grouped = new Map();
  (allContacts || []).forEach((contact) => {
    grouped.set(contact.id, { key: contact.id, contact, fallback: null, customers: [], roles: contact.position ? [contact.position] : [], primary: false });
  });
...
function paintContactTables(html, count) {
  ...
  if (directoryCount) directoryCount.textContent = `${count} contact di database`;
}
...
async function openContactEditor(contactId) {
  ...
  $('#deleteContactButton').style.display = isAdmin() ? '' : 'none';
...
$('#deleteContactButton').addEventListener('click', async () => {
  if (!isAdmin()) { showToast('Hanya administrator yang dapat menghapus contact.', true); return; }
  if (!editingContactId) return;
  const [customerLinks, vendorLinks] = await Promise.all([window.crmDb.getContactCustomers(editingContactId), window.crmDb.getContactVendors(editingContactId)]);
  const linked = (customerLinks.data?.length || 0) + (vendorLinks.data?.length || 0);
  if (linked > 0) { showToast(`Contact masih terhubung ke ${linked} relasi. Lepaskan dulu via daftar di bawah.`, true); return; }
  const name = $('#contactEditorForm input[name="name"]').value || 'contact ini';
  if (!window.confirm(`Hapus ${name} dari direktori?`)) return;
  const result = await window.crmDb.deactivateContact(editingContactId);
  if (result.error) { showToast(`Gagal menghapus: ${result.error.message}`, true); return; }
  closeContactEditor();
  showToast('Contact dihapus dari direktori.');
...
$$('.nav-item').forEach((item) => item.addEventListener('click', () => {
  ...
  if (module) {
    ...
    const isRelation = module === 'Relasi';
    ...
    $('#overviewView').hidden = isCompany || isAsset || isEmployee || isCustomer || isContact || isVendor || isRelation || isSales || isMaintenance || isWorkOrder || isInventory;
...
importModal.innerHTML = '<div class="modal relation-modal"><div class="modal-header"><div><p class="eyebrow">IMPOR DATA</p><h2>Impor customer dari CSV</h2></div><button class="icon-button" id="closeImportModal"><svg><use href="#i-close"/></svg></button></div><p class="modal-description">Export daftar pelanggan dari Accurate (atau Excel) ke CSV, lalu unggah di sini. Baris dengan nama yang sudah ada akan dilewati.</p><label>Pilih file CSV<input type="file" id="importFileInput" accept=".csv,text/csv" /></label><div id="importMappingWrap" hidden><div class="detail-section-heading"><h3>Pemetaan kolom</h3></div><div id="importMappingGrid"></div><div class="detail-section-heading"><h3>Pratinjau (5 baris pertama)</h3></div><div class="table-scroll"><table><thead><tr id="importPreviewHead"></tr></thead><tbody id="importPreviewRows"></tbody></table></div></div><div class="modal-actions"><button type="button" class="secondary-button" id="cancelImportModal">Batal</button><button class="primary-button" id="runImportButton" type="button" disabled>Impor sekarang</button></div></div>';
document.body.append(importModal);
applyTwoColumn(importModal, 720);
...
const importFieldAliases = {
  name: ['nama', 'name', 'nama pelanggan', 'nama customer', 'customer', 'pelanggan'],
  phone: ['telepon', 'telp', 'telpon', 'phone', 'no. telepon', 'no telepon', 'hp', 'no. hp', 'mobile'],
  email: ['email', 'e-mail', 'mail'],
  address: ['alamat', 'address', 'alamat lengkap'],
  city: ['kota', 'city', 'kabupaten'],
  npwp: ['npwp', 'no. npwp', 'tax id', 'nomor pajak'],
  contact: ['pic', 'kontak', 'contact', 'nama pic', 'contact person', 'cp']
};
const importFields = [['name', 'Nama customer *'], ['phone', 'Telepon'], ['email', 'Email'], ['address', 'Alamat'], ['city', 'Kota'], ['npwp', 'NPWP'], ['contact', 'PIC utama']];
const detectImportMapping = (header) => {
...
$('#importCustomerButton').addEventListener('click', () => {
  $('#importFileInput').value = '';
  $('#importMappingWrap').hidden = true;
...
$('#importFileInput').addEventListener('change', (event) => {
  ...
  reader.onload = () => {
    ...
    if (lines.length < 2) { showToast('File kosong atau hanya header.', true); return; }
    const delimiter = (lines[0].match(/;/g) || []).length > (lines[0].match(/,/g) || []).length ? ';' : ',';
    importHeaders = parseCsvLine(lines[0], delimiter);
    importRows = lines.slice(1).map((line) => parseCsvLine(line, delimiter));
    $('#importMappingGrid').innerHTML = importFields.map(([field, label]) => `<label>${label}<select data-map-field="${field}"><option value="">-- Abaikan --</option>${importHeaders.map((header, index) => `<option value="${index}"${detectImportMapping(header) === field ? ' selected' : ''}>${header}</option>`).join('')}</select></label>`).join('');
    $('#importPreviewHead').innerHTML = importHeaders.map((header) => `<th>${header}</th>`).join('');
    $('#importPreviewRows').innerHTML = importRows.slice(0, 5).map((row) => `<tr>${importHeaders.map((_, index) => `<td>${row[index] || ''}</td>`).join('')}</tr>`).join('');
    $('#importMappingWrap').hidden = false;
    $('#runImportButton').disabled = false;
    showToast(`${importRows.length} baris siap diimpor.`);
  };
  reader.readAsText(file);
});
$('#runImportButton').addEventListener('click', async () => {
  const mapping = {};
  $$('#importMappingGrid select').forEach((select) => { if (select.value !== '') mapping[select.dataset.mapField] = Number(select.value); });
  if (mapping.name === undefined) { showToast('Petakan kolom Nama customer dulu.', true); return; }
  $('#runImportButton').disabled = true;
  let saved = 0;
  let skipped = 0;
  let failed = 0;
  for (const row of importRows) {
    ...
    try {
      const existing = await window.crmDb.findCustomerByName(name);
      if (existing.error) throw existing.error;
      if (existing.data) { skipped += 1; continue; }
      const address = [get('address'), get('city')].filter(Boolean).join(', ') || null;
      const created = await window.crmDb.createCustomer({ customer_type: null, name, status: 'active', phone: get('phone') || null, email: get('email') || null, address, npwp: get('npwp') || null });
      if (created.error) throw created.error;
      if (get('contact')) {
        const contact = await window.crmDb.createContact({ full_name: get('contact'), phone: get('phone') || null, email: get('email') || null });
        if (!contact.error) await window.crmDb.createCustomerContactRelation({ customer_id: created.data.id, contact_id: contact.data.id, full_name: contact.data.full_name, phone: contact.data.phone, email: contact.data.email, is_primary: true });
      }
      saved += 1;
    } catch (err) {
      failed += 1;
  ...
  closeImportModal();
  showToast(`Impor selesai: ${saved} tersimpan, ${skipped} dilewati, ${failed} gagal.`);
...
$('#customerForm').addEventListener('submit', async (event) => {
  ...
  if (window.crmDb?.ready) {
    ...
    if (form.get('alsoVendor') && savedCustomer?.id) {
      const vendor = await window.crmDb.createVendor({ name: savedCustomer.name, phone: form.get('phone') || null, email: form.get('email') || null, address: form.get('address') || null });
      if (!vendor.error && vendor.data) {
        const link = await window.crmDb.createPartyLink({ customer_id: savedCustomer.id, vendor_id: vendor.data.id, notes: 'Otomatis: juga sebagai pemasok' });
        if (link.error && !String(link.error.message).includes('duplicate')) showToast(`Relasi pemasok gagal: ${link.error.message}`, true);
      }
    }
  }
  const name = form.get('name');
...
async function loadDatabaseData() {
  ...
  await reloadRelationDirectory();
  const { error: directoryError, count: contactCount } = await loadContactDirectory();
  ...
  const setNavBadge = (module, value) => {
    const badge = document.querySelector(`.nav-item[data-module="${module}"] em`);
    if (badge) badge.textContent = value;
  };
  if (!customerResult.error && customerResult.data) setNavBadge('Customer', customerResult.data.length);
  setNavBadge('Kontak', contactCount || '');
  if (!partsResult.error && partsResult.data) setNavBadge('Inventory', partCache.filter((part) => (Number(part.stock_on_hand) || 0) <= (Number(part.minimum_stock) || 0)).length || '');
  if (!scheduleResult.error && scheduleResult.data) setNavBadge('Maintenance', scheduleResult.data.length || '');
  if (!workOrderResult.error && workOrderResult.data) setNavBadge('SPK', workOrderResult.data.length || '');
  if (!customerResult.error && customerResult.data) {
...
async function openVendorDetail(vendorId) {
  ...
  $('#deleteVendorDetailButton').hidden = !isAdmin();
...
$('#deleteVendorDetailButton').addEventListener('click', async () => {
  if (!isAdmin()) { showToast('Hanya administrator yang dapat menghapus pemasok.', true); return; }
  const vendor = vendorCache.find((item) => String(item.id) === String(activeVendorId));
  if (!vendor) return;
  if (!window.confirm(`Hapus pemasok "${vendor.name}"? Riwayat PO dan harga tetap tersimpan. Customer yang tertaut tetap ada.`)) return;
  const result = await window.crmDb.deleteVendor(vendor.id);
  if (result.error) { showToast(`Gagal menghapus: ${result.error.message}`, true); return; }
  closeVendorDetail();
...
$('#vendorMasterForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (form.get('alsoCustomer') && result.data) {
      let customerId = null;
      const customer = await window.crmDb.createCustomer({ customer_type: null, name: result.data.name.toUpperCase(), status: 'active', phone: form.get('phone') || null, email: form.get('email') || null, address: form.get('address') || null });
      if (!customer.error && customer.data) customerId = customer.data.id;
      else if (customer.error && String(customer.error.message).includes('duplicate')) {
        const existing = await window.crmDb.findCustomerByName(result.data.name.toUpperCase());
        if (!existing.error && existing.data) customerId = existing.data.id;
      }
      if (customerId) {
        const link = await window.crmDb.createPartyLink({ customer_id: customerId, vendor_id: result.data.id, notes: 'Otomatis: juga sebagai customer' });
        if (link.error && !String(link.error.message).includes('duplicate')) showToast(`Relasi customer gagal: ${link.error.message}`, true);
      }
    }
    event.target.reset();
...
async function reloadRelationDirectory() {
  if (!window.crmDb?.ready) return;
  const [customers, vendors, links] = await Promise.all([window.crmDb.getCustomers(), window.crmDb.getVendors(), window.crmDb.getPartyLinks()]);
  if (customers.error || vendors.error || links.error) {
    $('#relationDirectoryCount').textContent = `Gagal memuat: ${(customers.error || vendors.error || links.error).message}`;
    return;
  }
  const linkedCustomers = new Set((links.data || []).map((link) => link.customer_id));
  const linkedVendors = new Set((links.data || []).map((link) => link.vendor_id));
  const rows = [];
  (customers.data || []).forEach((customer) => {
    const both = linkedCustomers.has(customer.id);
    const initials = customer.name.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase();
    rows.push(`<tr data-customer-id="${customer.id}"><td><div class="person"><div class="avatar avatar-blue">${initials}</div><div><b>${customer.name}</b><small>${customer.customer_code || ''}</small></div></div></td><td><span class="customer-type type-company">Customer</span>${both ? ' <span class="customer-type">+ Pemasok</span>' : ''}</td><td>${customer.phone || customer.email || '-'}</td><td><button class="more-button open-relation-customer" title="Buka customer"><svg><use href="#i-more"/></svg></button></td></tr>`);
  });
  (vendors.data || []).forEach((vendor) => {
    if (linkedVendors.has(vendor.id)) return;
    const initials = vendor.name.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase();
    rows.push(`<tr data-vendor-id="${vendor.id}"><td><div class="person"><div class="avatar avatar-orange">${initials}</div><div><b>${vendor.name}</b><small>${vendor.code || ''}</small></div></div></td><td><span class="customer-type">Pemasok</span></td><td>${vendor.phone || vendor.email || '-'}</td><td><button class="more-button open-relation-vendor" title="Buka vendor"><svg><use href="#i-more"/></svg></button></td></tr>`);
  });
  $('#relationDirectoryRows').innerHTML = rows.join('') || '<tr><td colspan="4">Belum ada data.</td></tr>';
  $('#relationDirectoryCount').textContent = `${(customers.data || []).length} customer · ${(vendors.data || []).length} pemasok · ${(links.data || []).length} merangkap keduanya`;
}
$('#relationDirectorySearch').addEventListener('input', (event) => {
  const query = event.target.value.toLowerCase();
  $$('#relationDirectoryRows tr').forEach((row) => { row.hidden = !row.textContent.toLowerCase().includes(query); });
});
$('#relationDirectoryRows').addEventListener('click', (event) => {
  const customerButton = event.target.closest('.open-relation-customer');
  if (customerButton) {
    const row = customerButton.closest('tr');
    const target = [...$$('#customerRows tr')].find((candidate) => candidate.dataset.customerId === row.dataset.customerId);
    if (target) openCustomerDetail(target);
    else showToast('Buka lewat halaman Customer untuk detail lengkap.', true);
    return;
  }
  const vendorButton = event.target.closest('.open-relation-vendor');
  if (vendorButton) openVendorDetail(vendorButton.closest('tr').dataset.vendorId);
});
const vendorImportModal = document.createElement('div');
vendorImportModal.className = 'modal-backdrop';
vendorImportModal.innerHTML = '<div class="modal relation-modal"><div class="modal-header"><div><p class="eyebrow">IMPOR DATA</p><h2>Impor pemasok dari file</h2></div><button class="icon-button" id="closeVendorImport"><svg><use href="#i-close"/></svg></button></div><p class="modal-description">Unggah file Excel (.xls/.xlsx) atau CSV daftar pemasok, misalnya daftar pemasok.xls dari Accurate.</p><label>Pilih file<input type="file" id="vendorImportFile" accept=".xls,.xlsx,.csv" /></label><div id="vendorImportMappingWrap" hidden><div class="detail-section-heading"><h3>Pemetaan kolom</h3></div><div id="vendorImportMappingGrid"></div><div class="detail-section-heading"><h3>Pratinjau (5 baris pertama)</h3></div><div class="table-scroll"><table><thead><tr id="vendorImportPreviewHead"></tr></thead><tbody id="vendorImportPreviewRows"></tbody></table></div></div><div class="modal-actions"><button type="button" class="secondary-button" id="cancelVendorImport">Batal</button><button class="primary-button" id="runVendorImportButton" type="button" disabled>Impor sekarang</button></div></div>';
document.body.append(vendorImportModal);
applyTwoColumn(vendorImportModal, 720);
const closeVendorImport = () => vendorImportModal.classList.remove('open');
$('#closeVendorImport').addEventListener('click', closeVendorImport);
$('#cancelVendorImport').addEventListener('click', closeVendorImport);
vendorImportModal.addEventListener('click', (event) => { if (event.target === vendorImportModal) closeVendorImport(); });
let vendorImportHeaders = [];
let vendorImportRows = [];
const vendorImportAliases = {
  name: ['nama', 'name', 'nama pemasok', 'nama vendor', 'nama supplier', 'vendor', 'pemasok', 'supplier'],
  phone: ['telepon', 'telp', 'telpon', 'phone', 'no. telepon', 'hp', 'no. hp', 'mobile', 'telp.'],
  email: ['email', 'e-mail', 'mail'],
  address: ['alamat', 'address', 'alamat lengkap'],
  city: ['kota', 'city', 'kabupaten'],
  npwp: ['npwp', 'no. npwp', 'tax id'],
  bank: ['bank', 'nama bank'],
  account: ['rekening', 'no. rekening', 'no rekening', 'account', 'bank account'],
  contact: ['pic', 'kontak', 'contact', 'nama pic', 'contact person', 'cp']
};
const vendorImportFields = [['name', 'Nama pemasok *'], ['phone', 'Telepon'], ['email', 'Email'], ['address', 'Alamat'], ['city', 'Kota'], ['npwp', 'NPWP'], ['bank', 'Bank'], ['account', 'No. rekening'], ['contact', 'PIC']];
$('#importVendorButton').addEventListener('click', () => {
  $('#vendorImportFile').value = '';
  $('#vendorImportMappingWrap').hidden = true;
  $('#runVendorImportButton').disabled = true;
  vendorImportModal.classList.add('open');
});
$('#vendorImportFile').addEventListener('change', async (event) => {
  const file = event.target.files[0];
  if (!file) return;
  try {
    let headers = [];
    ...
    if (/\.csv$/i.test(file.name)) {
      ...
      if (clean.length < 2) { showToast('File kosong atau hanya header.', true); return; }
      headers = clean[0].map((cell) => String(cell).trim());
      rows = clean.slice(1).map((row) => row.map((cell) => String(cell).trim()));
    } else {
      showToast('Pustaka Excel belum termuat. Coba lagi atau simpan sebagai CSV.', true);
      return;
    }
    vendorImportHeaders = headers;
    vendorImportRows = rows;
    const detect = (header) => {
      const normalized = header.toLowerCase().trim();
      for (const [field, aliases] of Object.entries(vendorImportAliases)) {
        if (aliases.some((alias) => normalized === alias || normalized.includes(alias))) return field;
      }
      return '';
    };
    $('#vendorImportMappingGrid').innerHTML = vendorImportFields.map(([field, label]) => `<label>${label}<select data-vendor-map="${field}"><option value="">-- Abaikan --</option>${headers.map((header, index) => `<option value="${index}"${detect(header) === field ? ' selected' : ''}>${header}</option>`).join('')}</select></label>`).join('');
    $('#vendorImportPreviewHead').innerHTML = headers.map((header) => `<th>${header}</th>`).join('');
    $('#vendorImportPreviewRows').innerHTML = rows.slice(0, 5).map((row) => `<tr>${headers.map((_, index) => `<td>${row[index] || ''}</td>`).join('')}</tr>`).join('');
    $('#vendorImportMappingWrap').hidden = false;
    $('#runVendorImportButton').disabled = false;
    showToast(`${rows.length} baris siap diimpor.`);
  } catch (err) {
    showToast(`File tidak dapat dibaca: ${err?.message || err}`, true);
  }
});
$('#runVendorImportButton').addEventListener('click', async () => {
  const mapping = {};
  $$('#vendorImportMappingGrid select').forEach((select) => { if (select.value !== '') mapping[select.dataset.vendorMap] = Number(select.value); });
  if (mapping.name === undefined) { showToast('Petakan kolom Nama pemasok dulu.', true); return; }
  $('#runVendorImportButton').disabled = true;
  let saved = 0;
  let skipped = 0;
  let failed = 0;
  for (const row of vendorImportRows) {
    const get = (field) => (mapping[field] !== undefined ? (row[mapping[field]] || '').trim() : '');
    const name = get('name');
    if (!name) { skipped += 1; continue; }
    try {
      const vendor = await window.crmDb.createVendor({ name, phone: get('phone') || null, email: get('email') || null, address: [get('address'), get('city')].filter(Boolean).join(', ') || null, npwp: get('npwp') || null, bank_name: get('bank') || null, bank_account_number: get('account') || null });
      if (vendor.error) throw vendor.error;
      if (get('contact')) {
        const relations = await window.crmDb.getVendorContacts(vendor.data.id);
        if (!relations.data?.some((rel) => (rel.contacts?.full_name || rel.full_name || '').toLowerCase() === get('contact').toLowerCase())) {
          const contact = await window.crmDb.createContact({ full_name: get('contact'), phone: get('phone') || null, email: get('email') || null });
          if (!contact.error) await window.crmDb.createVendorContactRelation({ vendor_id: vendor.data.id, contact_id: contact.data.id, full_name: contact.data.full_name, phone: contact.data.phone, email: contact.data.email, is_primary: true });
        }
      }
      saved += 1;
    } catch (err) {
