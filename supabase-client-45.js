(() => {
  ...
  window.crmDb = {
    ...
    async deactivateContact(id) {
      if (!this.ready) return { data: null, error: new Error('Supabase belum dikonfigurasi') };
      return window.supabaseClient.from('contacts').update({ is_active: false }).eq('id', id).select().single();
    },
    ...
    async getPartyLinks() {
      if (!this.ready) return { data: null, error: new Error('Supabase belum dikonfigurasi') };
      return window.supabaseClient.from('party_links').select('*, customers(id, name), vendors(id, name)').limit(1000);
    },
    async createPartyLink(payload) {
      if (!this.ready) return { data: null, error: new Error('Supabase belum dikonfigurasi') };
      return window.supabaseClient.from('party_links').insert(payload).select().single();
    },
    async deletePartyLink(id) {
      if (!this.ready) return { data: null, error: new Error('Supabase belum dikonfigurasi') };
      return window.supabaseClient.from('party_links').delete().eq('id', id);
    },
  ...
  if (window.crmDb.ready) {
    ...
    banner.style.cssText = 'position:fixed;top:14px;left:50%;transform:translateX(-50%);background:#fff3cd;border:1px solid #f0d486;color:#765b16;padding:10px 16px;border-radius:8px;font-size:12px;z-index:1000;max-width:92vw;text-align:center';
    banner.textContent = hasSdk ? 'Konfigurasi Supabase belum diisi. Aplikasi berjalan mode demo.' : 'Pustaka Supabase (CDN) gagal dimuat. Periksa koneksi, aplikasi berjalan mode demo.';
